...
JUPYTERHUB_PID=""
if kubectl get svc/proxy-public -n "$NAMESPACE" >/dev/null 2>&1; then
  kubectl port-forward svc/proxy-public 8181:80 -n "$NAMESPACE" &
  JUPYTERHUB_PID=$!
elif kubectl get svc/jupyterhub-proxy-public -n "$NAMESPACE" >/dev/null 2>&1; then
  kubectl port-forward svc/jupyterhub-proxy-public 8181:80 -n "$NAMESPACE" &
  JUPYTERHUB_PID=$!
fi
