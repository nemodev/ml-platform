...
- [ ] T024 [US1] Verify experiment logging from notebook: log in as user1, open notebook, run training script that calls `mlflow.set_experiment("iris-classification")`, `mlflow.log_param("n_estimators", 100)`, `mlflow.log_metric("accuracy", 0.95)`, `mlflow.sklearn.log_model(model, "model")`. Confirm the run is logged successfully
- [X] T025 [US1] Verify experiment retrieval via API: call `GET /api/v1/experiments` with user1's token, confirm "iris-classification" experiment is listed. Call `GET /api/v1/experiments/{id}/runs`, confirm the run with parameters and metrics is returned
...
- [X] T032 [US2] Verify MLflow UI embedding end-to-end: log in as user1, click "Experiments" in sidebar, confirm MLflow UI loads in iframe showing the "iris-classification" experiment, click on the experiment to see runs, select 2+ runs to compare metrics side-by-side, click a run to view artifact details
